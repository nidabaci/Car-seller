...
function showCars(cars) {
  if(!cars) return
    ...
    cars.forEach((car) => {
      ...
      <div class="col-12 col-md-4" id="${car.id}">

      <div class="shadow-sm p-3 mb-5 bg-body rounded car_shop ${car.status==" SOLD"?"soldCar":""}">
        <figure class="figure img_relativ w-100 mb-0">
          <p class="km"><span class="bullet"></span><span class='kmMi'> Km </span><span
              class='kmMi-value'>${car?.isKm0?"0":car.km}</span></p>
          <img src="${car.image}" class="figure-img img-fluid rounded" alt="...">
        </figure>
        <h3>${car.make}</h3>
        <p class="mb-0">${car.model}</p>
        <div class="ruler"></div>
        <div class=""> A partire da</div>
        <h4 class="d-flex justify-content-between align-items-center car-price">€ ${car.price}<ion-icon
            class='favorite ${favorites.includes(car.id)?"favorite-active":""}' id="fav-${car.id}"
            onclick="addFavorite(${car.id})" name="heart"></ion-icon>
        </h4>
        <div>
          <div class="details d-flex">
            <div class="car-specs">
              <strong>${car?.registrationYear ? car?.registrationYear +"-":""} <span
                  class='km_or_miles'>${car?.isKm0?0:car.km}</span>- Electro</strong>
              <p class="car-specs">Cons.Comb. Carburante: ${car.homologationStandard.wltp.consumption.combined}
                ${car.homologationStandard.wltp.consumption.unitOfMeasure} - ${car.co2}: 147 g/km </p>
            </div>
            <div>
